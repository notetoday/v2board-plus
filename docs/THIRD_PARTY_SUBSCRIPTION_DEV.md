...
- `vendor/bin/phpunit` 全量通过：**58 tests / 189 assertions**（截至 `f0ddef5`）
...
1. `app/Services/ThirdParty/ThirdPartySubscriptionService.php`：同步/缓存/合并逻辑
...
## 十二、测试环境数据（本地运行用，不进仓库）

- 管理员：`admin@test.local` / `admin123`
- 后端：`http://127.0.0.1:8001/603ea13d`
- 预览：`https://8001-3551905662e5d0f2.monkeycode-ai.live/603ea13d`（仅会话存活期间有效）
